...
def final_df() -> pl.LazyFrame:
    ...
    if FINAL_OUTPUT_PATH_PARTITIONED.is_dir() and list(FINAL_OUTPUT_PATH_PARTITIONED.glob("**/*.parquet")):
        ...
        pytest.skip(f"Final partitioned output data not found at {FINAL_OUTPUT_PATH_PARTITIONED}")
...
def assert_row_exists(
    ...
    if check_for_unique:
        assert num_rows == 1, f"Expected exactly 1 row for criteria {filter_criteria}, but found {num_rows}."
    else:
        assert num_rows > 0, f"Expected at least 1 row for criteria {filter_criteria}, but found none."
    ...
    for col, expected_val in expected_values.items():
        ...
        if is_expected_numeric and tolerance is not None and actual_numeric is not None:
            ...
            if expected_val is None:
                ...
                assert False, f"Column '{col}' mismatch: Expected {expected_val}, got None for criteria {filter_criteria}"
...
def test_final_output_values(final_df, source_iso, target_iso, hs_code_hs92, year_str, expected_value, expected_qty, expected_effective_tariff):
