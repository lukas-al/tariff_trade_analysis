...
def _(lf, pl, pycountry):
    ...
    aggregated_df = (
        lf.group_by("reporter_country")
        .agg(
            pl.len().alias("record_count"),  # Count rows per group
            pl.sum("value").alias("total_value"),  # Sum the 'volume' (value) per group
        )
        .with_columns(pl.col("reporter_country").map_elements(get_alpha_3, return_dtype=pl.Utf8).alias("reporter_country_alpha3"))
        .with_columns(pl.col("reporter_country").map_elements(get_country_name, return_dtype=pl.Utf8).alias("reporter_country_name"))
        .with_columns((pl.col("total_value") * 1000))
...
def _(aggregated_df, px):
    ...
    fig = px.scatter_geo(
        ...
        hover_data={"reporter_country_alpha3": True, "reporter_country": True, "record_count": ":,", "total_value": ":,.2f"},
    ...
    # Improve layout
    fig.update_layout(
        margin={"r": 0, "t": 50, "l": 0, "b": 0}, geo=dict(showland=True, landcolor="rgb(217, 217, 217)", subunitcolor="rgb(255, 255, 255)")
    )
...
def _(aggregated_df, go, make_subplots):
    ...
    bar_fig = make_subplots(rows=1, cols=2, subplot_titles=(f"Top {TOP_N} Countries by Total Export Value", f"Top {TOP_N} Countries by Record Count"))

    bar_fig.add_trace(
        go.Bar(
            y=top_value_df[label_col].to_list(), x=top_value_df["total_value"].to_list(), name="Total Value", orientation="h", marker_color="#FF6925"
        ),
        row=1,
        col=1,
    )
...
def _(get_alpha_3, go, lf, make_subplots, pl):
    def _():
        ...
        us_partner_agg_lf = (
            ...
            .agg(pl.sum("value").alias("total_value"), pl.len().alias("record_count"))
            .with_columns((pl.col("total_value") * 1000))
        )
        ...
        fig = make_subplots(
            rows=1,
            cols=2,
            subplot_titles=(f"Top {TOP_N_PARTNERS} US Partners by Total Export Value", f"Top {TOP_N_PARTNERS} US Partners by Record Count"),
        )
...
def _(get_alpha_3, go, lf, make_subplots, pl, plotly):
    def create_trade_charts():
        ...
        yearly_volume_lf = (
            lf.filter((pl.col("reporter_country") == US_NUMERIC_CODE_STR) & (pl.col("partner_country").is_in(top_n_partner_codes)))
            .group_by(["year", "partner_country"])
            .agg(pl.sum("value").alias("yearly_volume"))
            .sort("year", "partner_country")
        )  # Sort for plotting
        ...
        fig = make_subplots(
            ...
            cols=2,
            subplot_titles=(
                f"Top {TOP_N_PARTNERS} Partners by Total Export Value",
                f"Top {TOP_N_PARTNERS} Partners by Record Count",
                f"Yearly Export Volume for Top {TOP_N_PARTNERS} Partners by Value",
            ),
            specs=[
                [{}, {}],  # Row 1: Two normal subplots
                [{"colspan": 2}, None],
            ],  # Row 2: One subplot spanning 2 columns
        )
        ...
        if not yearly_volume_df.is_empty():
            ...
            for i, partner_alpha3 in enumerate(unique_partners):
                ...
                fig.add_trace(
                    ...
                    row=2,
                    col=1,  # Add to the subplot in row 2, column 1 (which spans)
                )
        else:
...
def _(get_alpha_3, go, lf, make_subplots, pl):
    def _():
        ...
        us_import_agg_lf = (
            ...
            .agg(  # Group by the reporting country (source of import)
                pl.sum("value").alias("total_value"), pl.len().alias("record_count")
            )
            .with_columns((pl.col("total_value") * 1000))
        )
        ...
        fig = make_subplots(
            ...
            cols=2,
            subplot_titles=(
                f"Top {TOP_N_SOURCES} Sources of US Imports by Total Value",
...
def _(distplot_effective_values, lf, pl):
    ...
    filtered_lf = lf.filter(~((pl.col("reporter_country").is_in(EU_COUNTRY_LIST)) & (pl.col("partner_country").is_in(EU_COUNTRY_LIST))))
