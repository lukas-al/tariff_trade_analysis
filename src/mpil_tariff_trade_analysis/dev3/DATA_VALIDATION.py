...
def _():
    ...
    from mpil_tariff_trade_analysis.utils.pipeline_funcs import vectorized_hs_translation  # noqa: F401
...
def _(
    ...
    mo.vstack(
        [
            mo.md(f"## Effective Tariff Chart \n HS code: {PRODUCT_CODE.value}, exporter {reporter_cc}, importer {partner_cc}"),
            px.line(product_ts, x="year", y="average_tariff_official", markers=True),
        ]
    )
    return
...
def _(np, pd):
    ...
    us_tariff_df = pd.concat([df_tariffs_comprehensive, df_new_hts_records], ignore_index=True)

    new_tariff_data = [
        {
            "Effective Date": pd.to_datetime("2025-02-05T00:00:00.000"),  # Entered on or after February 5, 2025
            "Action Entity": "USA",
        ...
        {
            "Effective Date": pd.to_datetime("2025-03-04T00:00:00.000"),  # Entered on or after March 4, 2025
            "Action Entity": "USA",
            "Tariff Target": "Products of China and Hong Kong (IEEPA)",
            "Legal Basis": "IEEPA",  # International Emergency Economic Powers Act
            "Trade Value (USD)": None,
    ...
    us_tariff_df = updated_df.sort_values(by=["Effective Date", "Action Entity", "HTS Heading", "Tariff Target"]).reset_index(drop=True)
...
def _(pd, pdfplumber, re):
    def extract_codes_from_pdf(pdf_path: str) -> pd.DataFrame:
        ...
        if not extracted_pairs:
            print(f"No HS code / Chapter 99 pairs found in PDF with the pattern: {pattern.pattern}")
            print("Please check the PDF structure or the regex if an empty DataFrame is unexpected.")
            return pd.DataFrame(columns=["Product HS Code", "Chapter 99 Heading"])

        df = pd.DataFrame(extracted_pairs, columns=["Product HS Code", "Chapter 99 Heading"])
        if not df.empty:
...
def _(code_df, map_tariff_rates, us_tariff_df):
    ...
    merged_tariff_df["Product HS6"] = merged_tariff_df["Product HS Code"].apply(lambda x: x.replace(".", "")[:6])
...
def _(merged_tariff_df, pd):
    official_us_hs6_tariffs = merged_tariff_df.groupby(["Product HS6", "Effective Date"])["Tariff Rate Applied"].mean().reset_index()

    official_us_hs6_tariffs["Effective Date"] = pd.to_datetime(official_us_hs6_tariffs["Effective Date"])

    official_us_hs6_tariffs["hs_revision"] = "HS6"
    official_us_hs6_tariffs.rename(columns={"Product HS6": "product_code"}, inplace=True)
...
@app.cell
def _(pl, unified_lf):
    filtered_lf = unified_lf.filter(pl.col("reporter_country") == "156", pl.col("partner_country") == "840")
...
def _(filtered_df, mo, pd, pl, remapped_df):
    ...
    for index, row in mo.status.progress_bar(remapped_df.iterrows(), title="Mapping values", total=len(remapped_df)):
        ...
        original_year_applied_from_remapped = pd.to_datetime(row["Effective Date"]).year
        ...
        wits_data = filtered_df.filter(
            (pl.col("product_code") == official_hs6_c_from_remapped) & (pl.col("year") == str(original_year_applied_from_remapped))
        )

        if wits_data.height > 0:
            matched_official_rows_count += 1  # Official row found at least one WITS match
            for wits_row_dict in wits_data.to_dicts():
                ...
                if wits_product_code.startswith("72"):
                    ...
                    tariff_rule_source = "Section 232 Steel (WITS Product Code Trigger)"
                elif wits_product_code.startswith("76"):
                    final_official_tariff = 0.10  # 10%
                    final_official_year = 2018
                    tariff_rule_source = "Section 232 Aluminium (WITS Product Code Trigger)"
    ...
    if results_list:
        ...
        results_df = results_df.with_columns(
            (pl.col("official_tariff_rate") * 100).alias("official_tariff_rate_percentage")
    ...
    print(f"Total rows processed from official tariffs data: {processed_rows_count}")
    print(f"Number of official tariff rows with matching WITS data: {matched_official_rows_count}")
    print(f"Number of official tariff rows with NO matching WITS data: {no_match_official_rows_count}")

    if results_df is not None:
        print(f"Total rows in the final combined DataFrame: {results_df.height}")
        print(f"Total individual WITS records matched and included: {total_wits_matches_collected}")
    else:
    ...
    if processed_rows_count != (matched_official_rows_count + no_match_official_rows_count):
        print("Warning: Discrepancy in processed row counts. This indicates an issue in the counting logic.")
    return (results_df,)
...
@app.cell
def _(px, results_df):
    px.histogram(results_df["average_tariff_official", "official_tariff_rate_percentage"])
    return
...
def _(filtered_df, pl, px, results_df):
    ...
    product_df = product_df.with_columns(pl.col("official_tariff_rate_percentage").forward_fill())

    product_df = product_df.with_columns(
        (pl.col("official_tariff_rate_percentage") - pl.col("average_tariff_official")).alias("difference_between_tariffs")
    )

    px.line(
        product_df,
        x="year",
        y=["average_tariff_official", "official_tariff_rate_percentage"],
    )
    return
...
@app.cell
def _(filtered_df, pl, px, results_df):
    official_tariffs_subset = results_df.select(["product_code", "year", "official_tariff_rate_percentage"])
    ...
    data_with_both_tariffs = filtered_df.join(official_tariffs_subset, on=["product_code", "year"], how="left")
    ...
    data_with_both_tariffs = data_with_both_tariffs.with_columns(
        ...
        .fill_null(0)  # If official_tariff rate is null (even after ffill for a product), set to 0
    )

    # Calculate the absolute discrepancy between the two tariff rates
    data_with_both_tariffs = data_with_both_tariffs.with_columns(
        (pl.col("official_tariff_rate_percentage") - pl.col("average_tariff_official")).abs().alias("absolute_discrepancy")
    )

    # Calculate the average absolute discrepancy for each year across all products
    yearly_average_discrepancy = (
        data_with_both_tariffs.group_by("year").agg(pl.mean("absolute_discrepancy").alias("average_abs_discrepancy")).sort("year")
    )
...
def _(pl, px, results_df, selected_df):
    official_and_wits_df = selected_df.join(
        other=results_df.select(["product_code", "year", "official_tariff_rate_percentage"]),
    ...
    official_and_wits_df = official_and_wits_df.with_columns(
        (pl.col("average_tariff_official") + pl.col("official_tariff_rate_percentage")).alias("full_tariff").cast(pl.Float32)
    )

    # Calculate total value per year for weighting
    total_value_per_year = official_and_wits_df.group_by("year").agg(pl.sum("value").alias("total_yearly_value"))

    # Calculate weighted tariffs
    weighted_tariffs_df = (
        official_and_wits_df.with_columns(
            (pl.col("average_tariff_official") * pl.col("value")).alias("weighted_effective_tariff_value"),
            (pl.col("full_tariff") * pl.col("value")).alias("weighted_full_tariff_value"),
        )
        .group_by("year")
        .agg(
            pl.sum("weighted_effective_tariff_value").alias("sum_weighted_effective"),
    ...
    final_weighted_tariffs_df = (
        ...
        .with_columns(
            (pl.col("sum_weighted_effective") / pl.col("total_yearly_value")).alias("trade_weighted_effective_tariff"),
            (pl.col("sum_weighted_full") / pl.col("total_yearly_value")).alias("trade_weighted_full_tariff"),
        )
        .select(
            [
                "year",
