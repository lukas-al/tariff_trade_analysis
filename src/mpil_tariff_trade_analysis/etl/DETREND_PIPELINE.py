...
def _(pl):
    def apply_detrending_to_lazyframe(
        ...
        global_avg_price_trend_lf = (
            ...
            .agg(pl.sum(source_value_col).alias(_sum_value_alias), pl.sum(source_quantity_col).alias(_sum_quantity_alias))
            .with_columns(
...
def _(pl, unified_lf_detrend):
    unified_lf_trf_detrend = unified_lf_detrend.with_columns(
        ...
        pl.col("unit_value_detrended").arcsinh().alias("unit_value_arcsinh_detrended"),
        pl.col("effective_tariff").arcsinh().alias("effective_tariff_arcsinh"),
    )

    return (unified_lf_trf_detrend,)


@app.cell
def _(unified_lf_trf_detrend):
    print("New unified_lf schema:", unified_lf_trf_detrend.collect_schema())
    return


@app.cell
def _(file_to_detrend_and_overwrite, pl, unified_lf_trf_detrend):
    # Sink output
    print("Sinking back to unified_lf")
    unified_lf_trf_detrend.sink_parquet(
        pl.PartitionByKey(
