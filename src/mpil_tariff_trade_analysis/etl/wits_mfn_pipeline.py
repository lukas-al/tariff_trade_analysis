...
def rename_wits_mfn_cleaned(df: pl.LazyFrame) -> pl.LazyFrame:
    """
    ...
    # Define expected columns after load_wits_tariff_data
    # It should have 'year', 'reporter_country_iso_numeric', 'product_code', 'tariff_rate', etc.
    # and 'hs_revision' (used for translation, maybe drop later), 'tariff_type'
    # The country remapping function `create_country_code_mapping_df` used in `load_wits_tariff_data`
    # produces list columns like 'reporter_country_iso_numeric' (List[Utf8]).
    # We need to handle this, perhaps by exploding if multiple mappings occurred (unlikely for reporter?)
    # or assuming a single value and taking the first element. Let's assume the latter for now.

    rename_map = {
        "year": "t", # Keep 't' for consistency during merge phase
        "reporter_country_iso_numeric": "i_list", # Keep list temporarily
        "product_code": "k", # HS code (already translated to HS92 by load_wits_tariff_data)
        "tariff_rate": "mfn_tariff_rate",
        "min_rate": "mfn_min_tariff_rate",
        "max_rate": "mfn_max_tariff_rate",
        # Keep tariff_type for potential debugging or filtering
    ...
    if missing_cols:
        logger.error(f"Cannot rename WITS MFN: Missing expected input columns: {missing_cols}. Available: {df.columns}")
    ...
    renamed_df = df.rename(rename_map).with_columns(
        # Take the first element of the list, assuming single reporter mapping
        pl.col("i_list").list.first().alias("i")
    ).select(select_cols + ["i"]) # Add the new 'i' column to selection
...
def validate_cleaned_wits_mfn(lf: pl.LazyFrame) -> bool:
    ...
    if schema.get("mfn_tariff_rate") != pl.Utf8:
         logger.warning(f"Validation Warning: MFN 'mfn_tariff_rate' column type is {schema.get('mfn_tariff_rate')}, expected Utf8.")
...
def run_wits_mfn_cleaning_pipeline(config: Dict[str, Any]) -> Optional[Path]:
    ...
    try:
        # load_wits_tariff_data handles loading, HS translation, and country remapping
        # It expects the base directory containing the AVEMFN/AVEPref subfolders
        # It returns a LazyFrame with columns like 'year', 'reporter_country_iso_numeric' (List[Utf8]), etc.
        loaded_df = load_wits_tariff_data(
            tariff_type="AVEMFN",
            base_dir=str(base_dir),
            # Pass mapping/ref paths if provided in config
        ...
        if loaded_df is None:
            logger.error("❌ Loading WITS MFN data returned None.")
            return None
        logger.info("✅ WITS MFN data loaded, HS translated, countries remapped.")
    ...
    # --- Step 2: Rename Columns & Handle List Columns ---
...
if __name__ == "__main__":
    ...
    test_config = {
        ...
        "WITS_MFN_CLEANED_OUTPUT_PATH": TEST_INTERMEDIATE_DATA_DIR / "cleaned_wits_mfn" / "WITS_AVEMFN_cleaned.parquet",
    ...
    if result_path:
        logger.info(f"WITS MFN cleaning pipeline test completed successfully. Output: {result_path}")
        sys.exit(0)
    else:
