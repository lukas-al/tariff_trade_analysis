...
def rename_wits_mfn_cleaned(input_lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    rename_map = {
        "year": "t",
        "reporter_country_iso_numeric": "i",  # Map the remapped column to 'i'
        "product_code": "k",
    ...
    #     if col in available_cols_after_rename or col in rename_map.values()
    # ]
    ...
    # Rename and select
    renamed_df = input_lf.rename(rename_map).select(select_cols)
...
def validate_cleaned_wits_mfn(lf: pl.LazyFrame) -> bool:
    ...
    try:
        ...
        if schema.get("t") != pl.Utf8:
            logger.error(
                f"Validation Error: MFN 't' column type is {schema.get('t')}, expected Utf8."
            )
            # return False # Allow flexibility if year becomes numeric later
        if schema.get("i") != pl.Utf8:
            logger.error(
                f"Validation Error: MFN 'i' column type is {schema.get('i')}, expected Utf8."
            )
            return False
        if schema.get("k") != pl.Utf8:
            logger.error(
                f"Validation Error: MFN 'k' column type is {schema.get('k')}, expected Utf8."
            )
            return False
        ...
        if schema.get("tariff_type") != pl.Utf8:
            logger.error(
                f"Validation Error: MFN 'tariff_type' column type is {schema.get('tariff_type')}, expected Utf8."
            )
            return False
...
def run_wits_mfn_cleaning_pipeline(config: Dict[str, Any]) -> Optional[Path]:
    ...
    try:
        current_lf = remap_codes_and_explode(
            ...
            output_column_names=["reporter_country_iso_numeric"],  # Specify the output name
...
#                 "Nbr_Free_Lines": [0],
#                 "Nbr_AVE_Lines": [1],
#                 "Nbr_Dutiable_Lines": [1],
#                 "TotalNoOfValidLines": [1],
#                 "TotalNoOfLines": [1],
#                 "EstCode": ["E"],
#             }
...
#         "BACI_REF_CODES_PATH": TEST_RAW_DATA_DIR
#         / "BACI_HS92_V202501"
...
#         pd.DataFrame({"country_code": ["840"], "country_name": ["United States"]}).to_csv(
#             test_config["BACI_REF_CODES_PATH"], index=False
#         )
#     if not Path(test_config["WITS_REF_CODES_PATH"]).exists():
#         pd.DataFrame({"ISO3": ["840"], "Country Name": ["United States"]}).to_csv(
#             test_config["WITS_REF_CODES_PATH"], index=False
#         )
#     # Create dummy HS mapping if needed
#     hs_map_path = Path(test_config["HS_MAPPING_DIR"]) / "H1_to_H0.CSV"
#     if not hs_map_path.exists():
#         pd.DataFrame({"H1_Code": ["010101"], "Desc": ["Dummy"], "H0_Code": ["010110"]}).to_csv(
#             hs_map_path, index=False, encoding="iso-8859-1"
#         )
