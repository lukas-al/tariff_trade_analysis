...
def expand_preferential_tariffs_pd(
    ...
    logger.debug(f"Input renamed_avepref shape: {renamed_avepref.shape}, dtypes:\n{renamed_avepref.dtypes}")
    logger.debug(f"Input pref_group_mapping shape: {pref_group_mapping.shape}, dtypes:\n{pref_group_mapping.dtypes}")
    ...
    logger.debug("Attempting left merge: renamed_avepref with pref_group_mapping on 'j' == 'region_code'.")
    try:
        ...
        joined_pref_mapping = pd.merge(
            ...
            how="left",
            validate="many_to_one" # Good practice to validate assumptions
        )
    ...
    try:
        ...
        mask = joined_pref_mapping['partner_list'].notna() & \
               joined_pref_mapping['partner_list'].apply(lambda x: isinstance(x, list) and len(x) > 0)
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"Head before explode (showing 'final_partner_list', Pandas):\n{joined_pref_mapping[['t', 'i', 'j', 'k', 'partner_list', 'final_partner_list']].head()}")
    except Exception as e:
    ...
    try:
        ...
        logger.error("CRITICAL: MemoryError occurred during explode operation (Pandas). The dataset is likely too large to fit in memory after exploding.", exc_info=True)
        raise
    except Exception as e:
...
def join_datasets_pd(
    ...
    logger.debug(f"Input renamed_avemfn shape: {renamed_avemfn.shape}, dtypes:\n{renamed_avemfn.dtypes}")
    logger.debug(f"Input expanded_pref shape: {expanded_pref.shape}, dtypes:\n{expanded_pref.dtypes}")
    ...
    try:
        ...
        for df_ref, keys in [(baci, pref_join_keys), (renamed_avemfn, mfn_join_keys), (expanded_pref, pref_join_keys)]:
                ...
                if key in df.columns and key in common_key_types:
                    ...
                    if target_type == 'int64' and current_type.lower().startswith('int'):
                        pass # Already an integer type
                         ...
                         logger.debug(f"Casting {key} in DataFrame from {current_type} to {target_type}")
    ...
    logger.debug(f"Merging intermediate result with expanded Preferential tariffs on {pref_join_keys}.")
    try:
        joined_all = pd.merge(joined_mfn, expanded_pref, on=pref_join_keys, how="left",
                              suffixes=('_mfn', '_pref')) # Add suffixes if any non-key columns overlap besides tariffs
    ...
    try:
        ...
        joined_all['effective_tariff_rate'] = joined_all['pref_tariff_rate'].fillna(joined_all['mfn_tariff_rate'])
...
def create_final_table_pd(joined_data: pd.DataFrame) -> pd.DataFrame:
    ...
    final_rename_map = {
        old: new for old, new in rename_map.items() if old in joined_data.columns
    }
...
def run_matching_pipeline_pd(
    ...
    try:
        ...
        logger.debug(f"Initial BACI shape: {baci_pd.shape}, Memory: {baci_pd.memory_usage(deep=True).sum() / 1024**2:.2f} MB")

        logger.info(f"Loading WITS MFN data from: {wits_mfn_path}")
        avemfn_pd = pd.read_parquet(wits_mfn_path)
        logger.debug(f"Initial WITS MFN shape: {avemfn_pd.shape}, Memory: {avemfn_pd.memory_usage(deep=True).sum() / 1024**2:.2f} MB")

        logger.info(f"Loading WITS Preferential data from: {wits_pref_path}")
        avepref_pd = pd.read_parquet(wits_pref_path)
        logger.debug(f"Initial WITS Pref shape: {avepref_pd.shape}, Memory: {avepref_pd.memory_usage(deep=True).sum() / 1024**2:.2f} MB")
    ...
    expanded_pref_pd = expand_preferential_tariffs_pd(renamed_avepref_pd, pref_group_mapping_pd) # Logging inside
    # Free up memory
    ...
    # Join datasets
    joined_data_pd = join_datasets_pd(baci_pd, renamed_avemfn_pd, expanded_pref_pd) # Logging inside
    # Free up memory
    del baci_pd
    ...
    try:
        ...
        logger.info(f"Pandas execution complete. Data saved. Final shape: {final_unified_table_pd.shape}")
    except Exception as e:
